jQuery(document).ready(function($) {
    $(".generate-more-images-button").click(function() {
        ...
        $.ajax({
            ...
            type: "POST",
            data: {
                action: "blogcopilot_io_generate_more_images",
                post_id: postId,
                title: postTitle, 
            ...
            success: function(response) {
                if (response.success) {                    
                    var imageCounter = $(`#ai_image_grid .ai_image_container`).length;
                    response.data.forEach(function(imageUrl) {
                        var confirmationId = "ai_confirmation_message_" + imageCounter;
                        ...
                        var newImageHtml = "<div class=\'ai_image_container\' style=\'text-align: center;\'>" +
                                            ...
                                            "</div>";
                        $(`#ai_image_grid`).append(newImageHtml);
                        imageCounter++;
                    });
                } else {
                    ...
                    var boxToReplace = $(`#blogcopilot_io_generate_more_images_container`);

                    boxToReplace.html(errorMessageHtml);                        
                }
            },
            error: function(response) {
                alert("Error generating images. Check server script running time.");
            },
            complete: function() {
